test_mct_bysite_alexi_fluxnet <- function(sitename, df_fluxnet, df_alexi, thresh_terminate = 0.2, thresh_drop = 0.8, use_return_period = 10, fittype = NULL){
  ...
  if (!dir.exists("fig/sif_cwd_alexi")) system("mkdir -p fig/sif_cwd_alexi")
  ...
  ## LUE vs CWD: ALEXI/WATCH-WFDEI------------------------------------------
    ...
    if (class(linmod) != "try-error"){
      ...
      if (class(linmod) != "try-error"){
        ...
        ## test: is slope negative?
        slope_lue_alexi <- coef(linmod)["deficit"]
        is_neg <- slope_lue_alexi < 0.0
    ...
    if (class(expmod) != "try-error"){
      ...
      ## test: is slope negative?
      k_lue_alexi <- df_coef["k", "Estimate"]
      is_neg_exp <- k_lue_alexi < 0
    ...
    if (nsif > 10){
      ...
      if (class(linmod) != "try-error"){
        ...
        if (class(linmod) != "try-error"){
          ...
          ## test: is slope negative?
          slope_sif_alexi <- coef(linmod)["deficit"]
          is_neg <- slope_sif_alexi < 0.0
          ...
          if (is_neg){
            ...
            if (is_sign){
              ## get x-axis cutoff
              sif0_alexi <- - coef(linmod)["(Intercept)"] / coef(linmod)["deficit"]
              df_fit <- tibble(y = predict(linmod, newdata = mct_alexi$df), x = mct_alexi$df$deficit)
            } else {
              sif0_alexi <- NA
            }
      ...
      if (class(expmod) != "try-error"){
        ...
        ## test: is slope negative?
        k_sif_alexi <- df_coef["k", "Estimate"]
        is_neg_exp <- k_sif_alexi < 0
        ...
        if (is_neg_exp){
          ...
          if (is_sign_exp){
            ...
            ## get CWD at wich LUE is at 1/2e
            sif0_exp_alexi <- -1.0/coef(expmod)["k"]*2.0   ## CWD where LUE is reduced to 1/2e
            df_fit_exp <- tibble(y = predict(expmod, newdata = mct_alexi$df), x = mct_alexi$df$deficit)
      ...
      if (is_sign){
        out <- out +
          geom_vline(xintercept = sif0_alexi, linetype = "dotted", color = 'tomato') +
          geom_line(data = df_fit, aes(x, y), col = "tomato")
      }
      if (is_sign_exp){
        out <- out +
          geom_vline(xintercept = sif0_exp_alexi, linetype = "dotted", color = 'royalblue') +
          geom_line(data = df_fit_exp, aes(x, y), col = "royalblue")
      }
      if (!identical(gumbi_alexi$mod, NA)){
        mctXX_alexi <- gumbi_alexi$df_return %>% dplyr::filter(return_period == use_return_period) %>% pull(return_level)
      ...
      out
      ggsave(paste0("fig/sif_cwd_alexi/sif_cwd_alexi_", sitename, ".pdf"), width = 6, height = 4)
  ...
  ## EOR vs CWD: ALEXI/WATCH-WFDEI------------------------------------------
    ...
    if (class(linmod) != "try-error"){
      ...
      if (class(linmod) != "try-error"){
        ...
        ## test: is slope negative?
        slope_eor_alexi <- coef(linmod)["deficit"]
        is_neg <- slope_eor_alexi < 0.0
    ...
    if (class(expmod) != "try-error"){
      ...
      ## test: is slope negative?
      k_eor_alexi <- df_coef["k", "Estimate"]
      is_neg_exp <- k_eor_alexi < 0
    ...
    out <- mct_alexi$df %>%
      mutate(iinst = as.factor(iinst)) %>% 
      ggplot() +
      geom_point(aes(x = deficit, y = eor, color = iinst), alpha = 0.5) +
  ...
  return(
    list(
      # gumbi_fluxnet = gumbi_fluxnet,  mct_fluxnet = mct_fluxnet, 
      mct_alexi = mct_alexi, gumbi_alexi = gumbi_alexi,
      # mctXX_fluxnet = mctXX_fluxnet, 
      mctXX_alexi = mctXX_alexi, 
      # lue0_fluxnet = lue0_fluxnet, lue0_exp_fluxnet = lue0_exp_fluxnet, 
      lue0_alexi = lue0_alexi, lue0_exp_alexi = lue0_exp_alexi,
      sif0_alexi = sif0_alexi, sif0_exp_alexi = sif0_exp_alexi,
      # eor0_fluxnet = eor0_fluxnet, eor0_exp_fluxnet = eor0_exp_fluxnet, 
      eor0_alexi = eor0_alexi, eor0_exp_alexi = eor0_exp_alexi,
      slope_lue_alexi = slope_lue_alexi, slope_eor_alexi = slope_eor_alexi,
      k_lue_alexi = k_lue_alexi, k_eor_alexi = k_eor_alexi
      )
    )
