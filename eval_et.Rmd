...
See `/alphadata01/bstocker/rsofun/vignettes/splash.Rmd` for how site-scale simulations were done.
...
ddf_modobs <- mod$daily %>% 
  ...
  right_join(
    ...
    by=c("sitename", "date")) %>%
  mutate(fpar_corr = (fpar - 0.1)/(1-0.1)) %>% 
...
ddf_modobs_evi <- mod_evi$daily %>% 
  ...
  right_join(
    dplyr::select(ddf_obs_dry, sitename, date, latenth_obs = latenth, netrad_obs = netrad),
    by=c("sitename", "date")) %>%
  mutate(evi_corr = (evi - 0.08)/(1-0.08)) %>% 
