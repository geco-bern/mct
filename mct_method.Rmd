...
df_example_alexi %>% 
  ...
  labs(x = "Date", y = expression(paste("Daily water balance (mm d"^{-1}, ")")), title = "Cumulative water balance" )
```
...
```{r}
# FLUXNET data
a <- get_plantwhc_mct_bysite(df_example_fluxnet, varname_wbal = "bal")
a$df_return

# ALEXI/WATCH-WFDEI data
b <- get_plantwhc_mct_bysite(df_example_alexi, varname_wbal = "bal")
b$df_return
```
...
ggplot() +
  ...
  geom_histogram(
    ...
    position="identity") +
  labs(title = "US-Ton", x = "Cumulative water deficit (mm)") +
  geom_vline(xintercept = mct_fluxnet$df_return %>% filter(return_period == 20) %>% pull(return_level), col = "black") +
  geom_vline(xintercept = mct_alexi$df_return %>% filter(return_period == 20) %>% pull(return_level), col = "tomato")
...
if (!identical(mct_alexi$mod, NA)){
  mct20 <- mct_alexi$df_return %>% filter(return_period == 20) %>% pull(return_level)
  if (!is.na(mct20)){
    out <- out +
...
df_corr_alexi <- purrr::map(out, "gumbi_alexi") %>% 
  purrr::map("df_return") %>% 
  purrr::map(~filter(., return_period==40) %>% pull(return_level)) %>% 
  tibble(
    sitename = names(.), 
    mct = unlist(.),
