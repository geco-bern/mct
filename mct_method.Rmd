...
df_fit = tibble(y = predict(linmod, newdata = out_mct_fluxnet$df_clean), x = out_mct_fluxnet$df_clean$deficit)

## Fit exponential
expmod <- nls(lue ~ lue0 * exp(k * deficit), data = out_mct_fluxnet$df_clean, start = list(lue0 = 0.1, k = 1/50))
out_mct_fluxnet$df_clean$lue_fit <- predict(expmod, newdata = out_mct_fluxnet$df_clean)
...
out_mct_fluxnet$df_clean %>% 
  ...
  geom_line(data = out_mct_fluxnet$df_clean, aes(x = deficit, y = lue_fit), color = 'royalblue')
```
...
## Fit exponential
expmod <- nls(lue ~ lue0 * exp(k * deficit), data = out_mct_alexi$df_clean, start = list(lue0 = 0.1, k = 1/50))
out_mct_alexi$df_clean$lue_fit <- predict(expmod, newdata = out_mct_alexi$df_clean)
lue0_exp <- -1/coef(expmod)["k"]*2
...
out_mct_alexi$df_clean %>% 
  ...
  geom_vline(xintercept = lue0_exp, linetype = "dotted", color = "royalblue") +
  geom_line(data = df_fit, aes(x, y), col = "red") +
  geom_line(data = out_mct_alexi$df_clean, aes(x = deficit, y = lue_fit), color = 'royalblue')
```
...
out_mct_fluxnet$df_clean$eor[idx_drop] <- NA

## git linear fit without outliers
linmod <- lm(eor ~ deficit, data = out_mct_fluxnet$df_clean)
...
## Fit exponential
expmod <- nls(eor ~ a * exp(k * deficit), data = out_mct_fluxnet$df_clean, start = list(a = 0.1, k = 1/50))
out_mct_fluxnet$df_clean$eor_fit <- predict(expmod, newdata = out_mct_fluxnet$df_clean)
...
out_mct_fluxnet$df_clean %>% 
  ...
  geom_line(data = df_fit, aes(x, y), col = "red") +
  geom_line(data = out_mct_fluxnet$df_clean, aes(x = deficit, y = eor_fit), color = 'royalblue') +
  ylim(0, 1)
```
...
out_mct_alexi$df_clean$eor[idx_drop] <- NA

## git linear fit without outliers
linmod <- lm(eor ~ deficit, data = out_mct_alexi$df_clean)
...
## get x-axis cutoff
eor0 <- - coef(linmod)["(Intercept)"] / coef(linmod)["deficit"]
df_fit = tibble(y = predict(linmod, newdata = out_mct_alexi$df_clean), x = out_mct_alexi$df_clean$deficit)

## Fit exponential
expmod <- nls(eor ~ a * exp(k * deficit), data = out_mct_alexi$df_clean, start = list(a = 0.1, k = 1/50))
out_mct_alexi$df_clean$eor_fit <- predict(expmod, newdata = out_mct_alexi$df_clean)
eor0_exp <- -1/coef(expmod)["k"]*2
...
out_mct_alexi$df_clean %>% 
  ggplot(aes(x = deficit, y = eor)) +
  geom_point(alpha = 0.5) +
  labs(title = "US-Ton", x = "Cumulative water deficit (mm)", y = "ET/Rn", subtitle = "ET: alexi, precipitation: alexi, Rn: alexi") +
  geom_vline(xintercept = eor0, linetype = "dotted", color = 'red') +
  geom_vline(xintercept = eor0_exp, linetype = "dotted", color = 'royalblue') +
  geom_line(data = df_fit, aes(x, y), col = "red") +
  geom_line(data = out_mct_alexi$df_clean, aes(x = deficit, y = eor_fit), color = 'royalblue') +
  ylim(0, 1)
```
