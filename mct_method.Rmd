...
```{r}
out_mct_fluxnet <- mct(df_example_fluxnet, varname_wbal = "bal", method = "threshbal", thresh_deficit=0.1 )
out_mct_alexi   <- mct(df_example_alexi,   varname_wbal = "bal", method = "threshbal", thresh_deficit=0.1 )
```
...
ggplot() +
  ...
  theme_classic() +
  labs(title = "ET and precipitation: FLUXNET", x = "Date", y = "Cumulative water deficit (mm)")
...
ggplot() +
  ...
  theme_classic() +
  labs(title = "ET: ALEXI, precipitation: WATCH-WFDEI", x = "Date", y = "Cumulative water deficit (mm)")
...
out_mct_fluxnet$df %>% 
  ...
  geom_smooth(aes(y =  gpp/ppfd), color = 'red', method = 'loess', se = TRUE) +
  ylim(-0.1, 0.3)
...
out <- list()
for (site in allsites[32:length(allsites)]){
