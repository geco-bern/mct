...
### Algorithm to identify CWD events

Get events of consecutive deficit using the `mct()` function.
```{r}
out_mct_fluxnet <- mct(df_example_fluxnet, varname_wbal = "bal", method = "threshbal", thresh_deficit=1.0 )
out_mct_alexi   <- mct(df_example_alexi,   varname_wbal = "bal", method = "threshbal", thresh_deficit=1.0 )
```
...
ggplot() +
  ...
  theme_classic() +
  labs(title = "FLUXNET 2015 data", x = "Date", y = "Cumulative water deficit (mm)")
...
ggplot() +
  ...
  coord_cartesian(ylim=c(0, 400)) + # , xlim = c(ymd("2002-01-01"), ymd("2010-12-01"))
  theme_classic() +
  labs(title = "ALEXI and WATCH-WFDEI", x = "Date", y = "Cumulative water deficit (mm)")
...
#   geom_line(data = out3$df, aes(date, prec), size = 0.3, color="royalblue") +
#   geom_line(data = out3$df, aes(date, surplus)) +
#   geom_line(data = out3$df, aes(date, surplus), color="tomato") +
#   coord_cartesian(ylim=c(0, 1000), xlim = c(ymd("2002-01-01"), ymd("2010-12-01"))) + #
#   theme_classic() +
...
```{r}
mct_alexi   <- get_plantwhc_mct_bysite(df_example_alexi,   varname_wbal = "bal", thresh_deficit = 1.0)
mct_fluxnet <- get_plantwhc_mct_bysite(df_example_fluxnet, varname_wbal = "bal", thresh_deficit = 1.0)
...
We expect that GPP, or GPP$/$PAR declines with increasing CWD. Let's align GPP at the onset of the largest few CWD events for one site.
...
df_example_alexi_tmp <- df_example_alexi %>% 
  ...
  ## merge FLUXNET GPP data into the alexi data frame
  left_join(
    df_example_fluxnet %>% 
      ungroup() %>% 
...
allsites <- ddf_fluxnet %>% pull(sitename)
out <- list()
