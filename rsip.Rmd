...
```{r}
df2 <- df_sites %>% 
  dplyr::select(sitename, Dr_sitemean = Dr, zroot_cwdx80_sitemean = zroot_cwdx80) %>% 
  right_join(
    df %>% 
      mutate(sitename = paste0("i_", as.character(lon), "_", as.character(lat))), 
    by = "sitename"
...
subset data to keep only those filtered above and for which at least five points per species remain. 51 species at the end, 402 data points.
```{r}
use_species2 <- df2 %>% 
...
df3 <- df2 %>% 
  dplyr::filter(Species %in% use_species & sitename %in% use_sites & Species %in% use_species2)
```
...
df_tga <- df3 %>% 
  ...
  group_by(Species) %>% 
  nest() %>% 
  mutate(linmod_n = purrr::map(data, ~lm(Dr ~ Dr_sitemean, data = .))) %>% 
...
df_tga %>% 
  ggplot() +
  geom_histogram(aes(slope_n, ..density..), fill = "royalblue", binwidth = 0.4, alpha = 0.5) +
...
df_tga %>% 
  ...
  mutate(Family = forcats::fct_reorder(Family, slope_n)) %>% 
  drop_na() %>% 
