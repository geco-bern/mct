...
```sh
src/ubelix/submit.sh watch_prec
...
`logs/ubelix/JOB_%A_%a.{out,err}`. Completed per-longitude products are skipped,
so an interrupted array can be submitted again. To rerun only missing indices,
derive them with `analysis/09_diagnostics/01_check_files.R` or inspect
