...
1. Run script `rscript_get_cwd.R` on Euler (submission file: `submit_get_cwd.R`) 
2. Run script `rscript_get_cwdx.R` on Euler (submission file: `submit_get_cwdx.R`).
...
Longitude slices without `bal` data. 174 additional ones not available!
```{r}
df %>% 
  dplyr::filter(!avl_bal & avl_et_mm)
```

Longitude slices without CWDX data. 480 additional ones not available!
```{r}
df %>% 
  dplyr::filter(!avl_cwdx & avl_bal & avl_et_mm)
```

Longitude slices without extracted CWDX data. 348 additional ones not available!
```{r}
df %>% 
