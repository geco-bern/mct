...
# ## central asia SIF
# gg <- df_cwd_lue0 %>% 
#   plot_map4(varnam = "cwd_lue0_SIF", lonmin = 45, lonmax = 95, latmin = 25, latmax = 47.5, 
#             breaks = c(seq(0, 100, by = 20), 150, 200, 300, 500, 700, 900, 1200, Inf), 
#             spacing = "constant",
#             combine = FALSE, 
#             colorscale = viridis::cividis, 
...
plot_map4(df_rl_agg_fet)
ggsave("fig/map_rl_fet.pdf", width = 12, height = 8)
```
...
df_rl_agg_test <- tmp_gti %>% 
  ...
  mutate(test = forestcoverfraction * gti) %>% 
  mutate(gti_bin = cut(gti, breaks = c(1:5, 10)),
         fcf_bin = cut(forestcoverfraction, seq(0, 1, by = 0.1)),
...
```{r}
df_rl_agg_test %>% 
  ggplot(aes(x = gti, y = ..count..)) +
  geom_histogram()

df_rl_agg_test %>% 
  ggplot(aes(x = forestcoverfraction, y = ..count..)) +
  geom_histogram()

df_rl_agg_test %>% 
  ggplot(aes(x = rp_diag, y = ..count..)) +
  geom_histogram()

df_rl_agg_test %>% 
  ggplot(aes(x = (WTD), y = ..count..)) +
  geom_histogram()

df_rl_agg_test %>% 
  ggplot(aes(x = test, y = ..count..)) +
  geom_histogram()
...
df_rl_agg_test %>% 
  drop_na(wtd_bin, rp_diag) %>% 
  ggplot(aes(x = wtd_bin, y = rp_diag)) +
  geom_boxplot()

df_rl_agg_test %>% 
  drop_na(gti_bin, rp_diag, forest) %>% 
  ggplot(aes(x = gti_bin, y = rp_diag, fill = forest)) +
  geom_violin()
```
...
Train linear regression model. WTD is not significant.
```{r}
print("-----with WTD-------")
...
gg80 <- out$gg +
  ...
  labs(x = expression(italic(S)[CWDX80] ~ " (mm)"), 
       y = expression(italic(S)[dnSIF] ~ " (mm)")) +
  scale_fill_gradientn(
        colours = colorRampPalette( c("gray65", "navy", "red", "yellow"))(5), 
...
```{r}
plot_grid(gg_fig1a, gg_fig1b, gg_fig1c, ncol = 1, labels = c('a', 'b', 'c'))
ggsave("fig/fig1.png", width = 8, height = 11)
```
