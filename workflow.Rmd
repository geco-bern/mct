...
save(df_modobs, file = "data/df_modobs_WF.Rdata")
```

Distribution by biomes.
```{r}
df_modobs %>% dplyr::select(sitename, biome_wwf, obs = D95_extrapolated, mod = zroot, lon = Longitude, lat = Latitude) %>% 
...
df_tmp <- df_modobs %>% 
  ...
  arrange(desc(error)) %>% 
  dplyr::select(sitename, zroot, D95_extrapolated, error, biome_wwf, lon = Longitude, lat = Latitude)
...
ggsave("fig/modobs_ridges_cwd_biome_wwf_NEW.pdf", width = 15, height = 10)
```
