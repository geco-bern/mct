...
S_dEF ~ GTI at regional scale?
```{r}
df_tmp <- df_cwd_et0 %>% 
  
  ## cut to central asia
  dplyr::filter(lon < 95 & lon > 45, lat > 25, lat < 47.5) %>% 
  left_join(df_irr, by = c("lon", "lat")) %>% 
  
  ## exclude irrigated areas
  dplyr::filter(irr < 0.3) %>% 
  
  ## add GTI info
  left_join(
    nc_to_df("~/data/gti_marthews/ga2_casia_05deg.nc", varnam = "Band1") %>% 
      rename(gti = Band1),
    by = c("lon", "lat"))

df_tmp %>% 
  mutate(gti_bin = cut(gti, breaks = 0:12)) %>% 
  ggplot(aes(x = gti_bin, y = cwd_lue0_fet)) +
...
```{r}
df_box <- tibble(
  long = c(45, 95, 95, 45, 45), lat = c(47.5, 47.5, 25, 25, 47.5), order = 1:5, group = rep(1, 5))
