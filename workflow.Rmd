...
nc_tenthdeg <- nc_to_df("data/cwdx80_tenthdeg.nc", varnam = "cwdx80") %>% 
  ...
  left_join(df_vegmask_tenthdeg %>%
              mutate(lon = round(lon, digits = 2), lat = round(lat, digits = 2)),
            by = c("lon", "lat")) %>%
  mutate(cwdx80 = ifelse(nonveg > 0.99, NA, cwdx80)) %>% 
...
# plot_grid(gg1, gg2, ncol = 1, labels = c('a', 'b'), rel_widths = c(1,1))
# ggsave("fig/map_whc_1m_2m.pdf", width = 10, height = 8)
# ggsave("fig/map_whc_1m_2m.png", width = 10, height = 8)
```
...
df_agg <- df %>% 
  group_by(lon_mid, lat_mid) %>% 
  summarise(deeper_1m = mean(deeper_1m, na.rm = TRUE), deeper_2m = mean(deeper_2m, na.rm = TRUE)) %>% 
  mutate(deeper_1m = ifelse(deeper_1m < 0.5, 0, 1), deeper_2m = ifelse(deeper_2m < 0.5, 0, 1)) %>% 
  rename(lon = lon_mid, lat = lat_mid)
```
...
plot_grid(gg1$ggmap, gg2$ggmap, gg2$gglegend, 
          gg3$ggmap, gg4$ggmap, gg4$gglegend,
          ncol = 3, labels = c('a', 'b', '', 'c', 'd', ''),
          rel_widths = c(1, 1, 0.12))
ggsave("fig/map_whc_1m_2m_deeper.png", width = 10, height = 5)
ggsave("fig/map_whc_1m_2m_deeper.pdf", width = 10, height = 5)
```
...
nc_tenthdeg <- nc_to_df("data/zroot_cwd80_tenthdeg.nc", varnam = "zroot_cwd80") %>% 
  ...
  left_join(df_vegmask_tenthdeg %>%
              mutate(lon = round(lon, digits = 2), lat = round(lat, digits = 2)),
            by = c("lon", "lat")) %>%
  mutate(zroot_cwd80 = ifelse(nonveg > 0.99, NA, zroot_cwd80)) %>%
...
# tmp %>%
#   plot_map4(varnam = "vegmask", lonmin = 45, lonmax = 95, latmin = 25, latmax = 47.5, 
#             spacing = "constant",
#             combine = TRUE, 
#             colorscale = "batlowK", legend_title = "(mm)",
#             expand_size_y = 0.5,
...
cowplot::plot_grid(gg_fig3a, gg_fig3a_legend, 
                   gg_fig3b, gg_fig3b_legend, 
                   ncol = 2, rel_widths = c(1, 0.12), labels = c('a', '', 'b', ''))
ggsave("fig/fig3.png", width = 10, height = 8) # height = 11 when including the histograms
ggsave("fig/fig3.pdf", width = 10, height = 8) # height = 11 when including the histograms
```
