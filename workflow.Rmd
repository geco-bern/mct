...
## central asia EF
gg <- df_cwd_et0 %>% 
  plot_map4(varnam = "cwd_lue0_fet", lonmin = 45, lonmax = 95, latmin = 25, latmax = 47.5, 
            breaks = c(seq(0, 100, by = 20), 150, 200, 300, 500, 700, 900, 1200, Inf), 
            spacing = "constant",
            combine = FALSE, 
            colorscale = viridis::cividis, 
...
ggsave("fig/map_cwd_lue0_fet0_casia2.png", width = 10, height = 6)

## central asia groundwater
gg <- df_cwd_et0 %>%
  mutate(groundwater = ifelse(type_fet %in% "A2", "A", "B")) %>% 
  # mutate(groundwater = ifelse(type_fet %in% c("Aa", "Ab", "A1", "A2a"), 
  #                             "A", 
  #                             ifelse(type_fet %in% c("Ba", "Bb", "B1a", "B1"), 
  #                                    "B", "C"))) %>%  # 
  plot_map4(varnam = "groundwater", lonmin = 45, lonmax = 95, latmin = 25, latmax = 47.5,
            spacing = "constant",
            breaks = 0:2, #  0:3,
            combine = TRUE,
...
ggsave("fig/map_types_casia2.pdf", width = 10, height = 6)
ggsave("fig/map_types_casia2.png", width = 10, height = 6)
...
For each gridcell, use fitted Gumbel distribution to calculate return level for a set of return periods. This is done with `rscript_return_level.R`. 

Then calculate the bias with EF and SIF-based S0 for each return period.

```{r}
load("data/df_rp_diag_nSIF.RData")
load("data/df_rp_diag_fet.RData")
...
df_rp_diag_fet %>% 
  ...
  ylim(0, 200)
```
Convert to map, regrid, and plot.
```{r}
nc <- df_to_grid(df_rp_diag_nSIF, varnam = "rp_diag", lonnam = "lon", latnam = "lat")
write_nc2(nc, varnams = "rp_diag", lon = df_rp_diag_nSIF$lon %>% unique() %>% sort(), lat = df_rp_diag_nSIF$lat %>% unique() %>% sort(), path = "data/return_period_nSIF.nc", make_zdim = FALSE)

system("cdo remapbil,gridfile_tenthdeg.txt data/return_period_nSIF.nc data/return_period_nSIF_tenthdeg.nc")

nc <- read_nc_onefile("data/return_period_nSIF_tenthdeg.nc", varnam = "rp_diag")
gg <- plot_map3(nc, varnam = "rp_diag", breaks = c(seq(0, 200, by = 20), Inf), latmin = -55, latmax = 80,
          spacing = "constant", combine = FALSE, colorscale = viridis::cividis, legend_title = "(yr)")
gg$ggmap <- gg$ggmap + 
  labs(title = expression(italic(T)[nSIF]))
cowplot::plot_grid(gg$ggmap, gg$gglegend, ncol = 2, rel_widths = c(1, 0.2))
ggsave("fig/map_return_period_nSIF.pdf", width = 10, height = 6)
ggsave("fig/map_return_period_nSIF.png", width = 10, height = 6)
```
