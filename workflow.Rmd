...
XXXXX THIS DOESNT' WORK - SHOULD DO THE SAME WAY AS BEFORE: WITH PURRR::MAP
...
Get daily water balance and reduce data frames.
```{r}  
filn <- paste0("data/df_bal_", siteset, ".RData")
if (!file.exists(filn)){
  
  # ## test plot visualising snow accumulation and mel
  # df_clean$data[[1]] %>% 
  ...
  df_bal <- df_clean %>%
    ...
    dplyr::mutate( data = purrr::map(data, ~cutna_headtail_df(., "bal")))
