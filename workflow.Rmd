...
save(df, file = "data/df_zroot.RData")
```
...
```{r}
library(ggridges)
df_modobs %>% dplyr::select(sitename, biome_wwf, obs = D95_extrapolated, mod = zroot_cwdx20, lon, lat) %>%  # change to mod = zroot_cwdx20_global
...
gg$ggmap <- gg$ggmap + 
  labs(title = "Maximum cumulative water deficit", subtitle = "Estimated by dSIF") +
...
gg$ggmap <- gg$ggmap + 
  labs(title = "Maximum cumulative water deficit", subtitle = "Estimated by dnSIF") +
  geom_path(data = df_rivers, aes(x = long, y = lat, group = group), color = "dodgerblue")
cowplot::plot_grid(gg$ggmap, gg$gglegend, ncol = 2, rel_widths = c(1, 0.2))
...
gg <- plot_map3(nc_halfdeg, varnam = "cwd_lue0_SIF", breaks = c(seq(0, 100, by = 20), 150, 200, 300, 500, 1000, Inf), latmin = -55, latmax = 80,
          spacing = "constant", maxval = 6000, combine = FALSE, colorscale = viridis::viridis, legend_title = "(mm)")
gg$ggmap <- gg$ggmap + 
...
gg <- plot_map3(nc_halfdeg, varnam = "cwd_lue0_nSIF", breaks = c(seq(0, 100, by = 20), 150, 200, 300, 500, 1000, Inf), latmin = -55, latmax = 80,
          spacing = "constant", maxval = 6000, combine = FALSE, colorscale = viridis::viridis, legend_title = "(mm)")
gg$ggmap <- gg$ggmap + 
  labs(title = "Maximum cumulative water deficit", subtitle = "Estimated by dSIF")
cowplot::plot_grid(gg$ggmap, gg$gglegend, ncol = 2, rel_widths = c(1, 0.2))
...
df_cluster_SIF$cluster <- out$cluster
...
## central asia
gg <- df_cluster_SIF %>% 
  plot_map3(varnam = "cluster", lonmin = 45, lonmax = 95, latmin = 25, latmax = 47.5, 
            breaks = 0:3,
            spacing = "constant",
            combine = FALSE,
...
```{r}
## ET
nc <- df_to_grid(df, varnam = "cwd_lue0_et", lonnam = "lon", latnam = "lat")
write_nc2(nc, varnams = "cwd_lue0_et", lon = df$lon %>% unique() %>% sort(), lat = df$lat %>% unique() %>% sort(), path = "data/cwd_lue0_et.nc", make_zdim = FALSE)

## fET (ET/Rn)
nc <- df_to_grid(df, varnam = "cwd_lue0_fet", lonnam = "lon", latnam = "lat")
write_nc2(nc, varnams = "cwd_lue0_fet", lon = df$lon %>% unique() %>% sort(), lat = df$lat %>% unique() %>% sort(), path = "data/cwd_lue0_fet.nc", make_zdim = FALSE)

## s0_teuling_et
nc <- df_to_grid(df %>% mutate(s0_teuling_et = remove_outliers(s0_teuling_et, coef = 10)), varnam = "s0_teuling_et", lonnam = "lon", latnam = "lat")
write_nc2(nc, varnams = "s0_teuling_et", lon = df$lon %>% unique() %>% sort(), lat = df$lat %>% unique() %>% sort(), path = "data/s0_teuling_et.nc", make_zdim = FALSE)
```
...
plot_map3(nc_halfdeg, varnam = "cwd_lue0_et", breaks = c(seq(0, 100, by = 20), 150, 200, 300, 500, 1000, Inf), latmin = -55, latmax = 80,
          spacing = "constant", maxval = 6000)
...
plot_map3(nc_halfdeg, varnam = "cwd_lue0_fet", breaks = c(seq(0, 100, by = 20), 150, 200, 300, 500, 1000, Inf), latmin = -55, latmax = 80,
          spacing = "constant", maxval = 6000)
...
plot_map3(nc_halfdeg, varnam = "s0_teuling_et", breaks = c(seq(0, 100, by = 20), 150, 200, 300, 500, 1000, Inf)*10, latmin = -55, latmax = 80,
          spacing = "constant", maxval = 20000)
...
gg$ggmap <- gg$ggmap + 
  labs(title = "S0 after Teuling et al.") +
  geom_path(data = df_rivers, aes(x = long, y = lat, group = group), color = "dodgerblue")
