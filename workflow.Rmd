...
write_nc2(nc, varnams = "cwdx20", lon = df$lon %>% unique() %>% sort(), lat = df$lat %>% unique() %>% sort(), path = "data/cwdx20.nc")
```
...
```{r}
df %>% dplyr::filter(cwdx20 < 800) %>% ggplot(aes(x = cwdx20, y = ..density..)) + geom_density()

# df %>% 
#   dplyr::filter(cwdx20 < 800) %>% 
#   tidyr::pivot_longer(cols = c(cwdx10, cwdx20, cwdx40), names_to = "return_period", values_to = "cwdxx") %>% 
#   ggplot(aes(x = cwdxx, y = return_period)) +
#   geom_density_ridges(
...
gg <- df %>% 
  ...
  plot_map3(varnam = "cwdx20", lonmin = -7, lonmax = -4, latmin = 36, latmax = 37.5, combine = FALSE)
