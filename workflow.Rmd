...
nc_tenthdeg <- nc_to_df("data/cwdx80_tenthdeg.nc", varnam = "cwdx80") %>% 
  mutate(cwdx80 = ifelse(lat > 70, NA, cwdx80))
...
nc_tenthdeg <- nc_to_df("data/zroot_cwd80_tenthdeg.nc", varnam = "zroot_cwd80") %>% 
  mutate(zroot_cwd80 = ifelse(lat > 70, NA, zroot_cwd80)) %>% 
  # mutate(lon = round(lon, digits = 2), lat = round(lat, digits = 2)) %>% 
  # left_join(df_vegmask_tenthdeg %>% 
  #             mutate(lon = round(lon, digits = 2), lat = round(lat, digits = 2)), 
  #           by = c("lon", "lat")) %>% 
...
## write as netcdf
lon_orig <- df_cwd_et0$lon %>% unique() %>% sort()
lon <- seq(min(lon_orig), max(lon_orig), by = 0.05)

lat_orig <- df_cwd_et0$lat %>% unique() %>% sort()
...
nc <- df_to_grid(df_tmp, varnam = "flat_fet", lonnam = "lon", latnam = "lat")
write_nc2(nc, 
          varnams = "flat_fet", 
          lon = lon, 
          lat = lat, 
...
ggsave("fig/fig_return_period.png", width = 8, height = 10)
ggsave("fig/fig_retrun_period.pdf", width = 8, height = 10)
...
## fig 3
cowplot::plot_grid(gg_fig3a, gg_fig3a_legend, gg_fig3b, gg_fig3b_legend, 
