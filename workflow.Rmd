...
Along one longitude band (21.475 deg E).
```{r}
load("~/mct/data/df_cwd_et0/df_cwd_et0_4030.RData")
df_cwd_et0 <- df
load("data/df_cwdx/df_cwdx_ilon_4030.RData")
tmp <- df %>% 
  mutate(df_return = map(out_mct, "df_return")) %>% 
  mutate(cwdx20 = map_dbl(df_return, ~{filter(., return_period == 200) %>% pull(return_level)}))

ggplot() + 
  geom_line(data = df_cwd_et0, aes(lat, cwd_lue0_fet), color = "red") +
  geom_line(data = tmp, aes(lat, cwdx20))
...
tmp <- tmp %>% 
  ...
  filter(flue_fet < 0.7) %>% 
  mutate(lambda_decay_et = remove_outliers(lambda_decay_et)) %>% 
  mutate(cwd_lue0_fet = remove_outliers(cwd_lue0_fet)) %>% 
  mutate(cwd_lue0_et = remove_outliers(cwd_lue0_et))

tmp %>% 
  analyse_modobs2("cwdx20", "cwd_lue0_fet")
tmp %>% 
  analyse_modobs2("cwdx20", "cwd_lue0_et")
```
