...
df_test_mct_gg <- df_mct_sj02_alexi %>% 
  mutate(mct = purrr::map(out_mct_00_80, "mct")) %>% 
  mutate(df  = purrr::map(mct, "df")) %>% 
  dplyr::select(idx, lon, lat, mct) %>% 
  mutate(gg_cwd_tseries = purrr::map(mct, ~try(test_cwd_tseries(., filter_years = c(2010, 2011, 2012)))))
...
df_wrongsites <- df_modobs %>% 
  dplyr::filter(D95_extrapolated < 2500) %>% 
...
df_test_gg <- df_mct_sj02_alexi %>% 
  ...
  mutate(mct = purrr::map(out_mct_10_90, "mct")) %>% 
