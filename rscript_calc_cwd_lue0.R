...
if (ncores > 1){
  ...
  cl <- multidplyr::new_cluster(ncores) %>%
    multidplyr::cluster_library(c("dplyr", "purrr", "tidyr", "dplyr", "magrittr", "lubridate", "rlang", "rbeni")) %>%
  ...
  df_out <- tibble(ilon = irow_chunk[[as.integer(args[1])]]) %>%
    ...
    dplyr::mutate(out = purrr::map( ilon,
                                    ~try(calc_cwd_lue0_byilon(.))))

} else {

  ## testing
  df_out <- purrr::map(as.list(irow_chunk[[as.integer(args[1])]]), ~try(calc_cwd_lue0_byilon(.)))
